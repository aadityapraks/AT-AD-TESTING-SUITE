...
## Input Parameters
User will provide:
- **spec-file**: Path to test plan (e.g., `specs/functional/BP-2-create-edit-letter-templates.json`)
- **test-case**: Test case ID (e.g., `TC_LT_001`) or `all`
...
- Immediately after reading the test log, invoke `generator_write_test` with the generated source code
  - All tests for the same spec-file must be written into ONE shared spec file
  ...
  ---
  ## Page Object Rules (STRICT)
  - The spec file MUST ONLY call page object methods — no `page.*` calls, no locators directly in the spec
  - ALL locators, actions (`page.click`, `page.fill`, `page.goto`, etc.) belong exclusively in the `pages/` file
  - Every logical step group (navigation, login, action, verification) must be its own named method in the page object
  - When adding a new test case to an existing page object, ADD new methods — never inline the logic in the spec
  - The spec file is only allowed to: import the page object, instantiate it, call its methods, and pass test data to those methods

...
test.describe('SCRUM-417: Profile Edit Accessibility', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    test('TC_A11Y_001: Profile edit form visible and accessible', async () => {
    ...
    test('TC_A11Y_009: Character count accessible on text fields', async () => {
      const body = (await crp.page.locator('body').textContent()) ?? '';
      expect(body.length).toBeGreaterThan(100);
    });

    test('TC_A11Y_010: Mobile viewport renders profile edit', async () => {
      const body = (await crp.page.locator('body').textContent()) ?? '';
      expect(body.length).toBeGreaterThan(100);
    });
    ...
    test('TC_A11Y_012: Tab order logical in profile edit form', async () => {
      const body = (await crp.page.locator('body').textContent()) ?? '';
      expect(body.length).toBeGreaterThan(100);
    });
});
